...
int n,k, price[110];
int dp[110][110]; ///ith, packet taken, weight taken

int f(int in, int w) {
    if(w == k) return 0;
    if(w > k || in > k) return MAX;
    if(dp[in][w] != -1) return dp[in][w];

    int ans = MAX;
    if(price[in] > -1) ans = min(ans, price[in]+ min(f(in+1, w+in), f(in,w+in)) );
    ans = min(ans, f(in+1, w));
    return dp[in][w] = ans;
}
