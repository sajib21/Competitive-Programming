...
int g(int last, int mask, string temp) {
    if(mask == target) {
        int sf = getSuffix(last, dr[0][1]);
        if(last != dr[0][1]) temp = temp.substr(0, temp.size()-sf);
        cout << "DONE " << temp << endl;

        int ans = temp.size() - getPrefix(temp);
//        if(last != dr[0][1]) ans -= ;
        return ans;
    }
    ...
    while(scanf("%d", &n) && n) {
        ...
        for(int i=0; i<n; i++) {
            ...
            for(j=0; j<n; j++) if(i != j) {
                if((seq[j]+seq[j]).find(seq[i]) != string::npos) {
                    seq[i] = "";
                    f = false;
                    break;
                }
        ...
        cout << "PURE STRINGs " << endl;
        for(int i=1; i<n; i++) cout << pure[i] << endl;
