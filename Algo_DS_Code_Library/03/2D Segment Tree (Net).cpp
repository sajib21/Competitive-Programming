...
class segtree_y{
    ...
    inline void update(int node, int s, int e, int i, int j, long long val){
        ...
        within[node] ^= ((min(e, j) - max(i, s) + 1)%2) * val;
    }

    inline long long query(int node, int s, int e, int i, int j){
        if(s > j or e < i) return 0;
        if(s >= i and e <= j) return (1LL * whole[node] * ((e - s + 1)%2)) ^ within[node];
        int mid = (s + e) >> 1;
        int lc = (node + 1) * 2 - 1;
        int rc = lc + 1;
        return query(lc, s, mid, i, j) ^ query(rc, mid + 1, e, i, j) ^ (whole[node] * ((min(e,j) - max(s, i) + 1)%2) );
    }
...
class segtree_x{
    ...
    inline void update(int node, int s, int e, int i, int j, int b1, int b2, long long val){
        ...
        within[node].update(b1, b2, val * ((min(j, e) - max(i, s) + 1)%2));
    }
