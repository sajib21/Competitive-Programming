...
//               ( upper_bound(dis[v].begin(), dis[v].end(), W-getDist(U,u) ) - dis[v].begin() );

        ans += ( upper_bound(dis[u].begin(), dis[u].end(), W-getDist(U,u) ) - dis[u].begin() ) -
               ( upper_bound(badd[u][badid].begin(), badd[u][badid].end(), W-getDist(U,u) ) - badd[u][badid].begin() );

//        int pp = upper_bound(dis[u].begin(), dis[u].end(), W-getDist(U,u) ) - dis[u].begin() ;
//        int qq = upper_bound(dis[v].begin(), dis[v].end(), W-getDist(U,u) ) - dis[v].begin() ;
//
//        cout << "IN " << u << ' ' << v << ' ' << getDist(U,u) << ' ' << getDist(U,u) << ' ' << pp << ' ' << qq << endl;
...
//    printCenTree();
    ...
    for(int u=1; u<=n; u++) {
        ...
        //sort(id[u].begin(), id[v].end())
    }

//    cout << "INIT " << endl;
//    for(int i=1; i<=n; i++) {
//        cout << i << " : ";
//        for(int j : dis[i]) cout << ' ' << j ;
//        cout << endl;
//    }
...
//        printf("%d\n", query(u, w));
        printf("%d\n", brute(u, w));
//        cout << "Q " << query(u,w) << endl;
//        cout << "B " << brute(u, w) << endl;
    }
